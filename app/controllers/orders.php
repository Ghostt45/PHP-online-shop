...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['button-create-cart-order'])) {


    if (!empty($_SESSION['cart'])) {
        foreach ($_SESSION['cart'] as $cart_item) {
            $id_post = $cart_item['id'];
        ...
        if (empty($errMsg)) {
            mysqli_query($connect, "INSERT INTO `orders` (`id`, `id_post`, `username`, `quantity`, `price`, `status`, `create_date`) VALUES (NULL, '$id_post', '$username', '$all_quantity', '$price', '$status', CURRENT_TIMESTAMP)");
